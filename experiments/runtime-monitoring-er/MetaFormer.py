...
if __name__ == "__main__":
    weights = r"C:\Users\20182054\OneDrive - TU Eindhoven\PhD\code\intrasurge\runtime_verification_experiment\CAFormerS18_RAMIE_SurgeNet.pth"
    weights = torch.load(weights)


    # For Full Segmentation model
    model = MetaFormerFPN(num_classes=4, pretrained='ImageNet', weights=weights).cuda() # pretrained {ImageNet, SurgNet}
    dummy = torch.zeros([12, 3, 256, 256]).cuda()
    out = model(dummy)
    print(out.shape)


    model = caformer_s18(num_classes=4, pretrained='ImageNet', weights=weights).cuda()
    dummy = torch.zeros([12, 3, 128, 128]).cuda()
    out = model(dummy)
    for i in out:
        print(i.shape)
